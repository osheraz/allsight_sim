...
class allsight_renderer:
    def __init__(
        ...
        bg = cv2.imread(os.path.join(PATH, f"experiments/conf/ref/ref_frame_{leds}15.jpg"))
        ...
        self.renderer = allsight_wrapper.Sensor(
    ...
    def update_pose_given_point(self, point, press_depth, shear_mag, delta=0.0):
        ...
        delta = 0 # np.random.uniform(low=0.0, high=2 * np.pi, size=(1,))[0]
...
def main(cfg: DictConfig) -> None:
    ...
    tac_render = allsight_renderer(cfg, obj_path, randomize=False)
    ...
    for vertix_idx in vertix_idxs:
        ...
        tac_render.update_pose_given_point(vertix_idx,
                                           press_depth,
                                           shear_mag=0.0)
