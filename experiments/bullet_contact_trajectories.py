...
class SimContactTrajectories:
    ...
    def load_scene(self, obj_name=None, scene_id=None):
        ...
        self.objects_scene[obj_name] = self.bullet_client.loadURDF(fileName=self.cfg[obj_name]["urdf_path"],
                                                                   ...
                                                                   baseOrientation=self.cfg[obj_name][
                                                                       "base_orientation"],
                                                                   useMaximalCoordinates=False,
        ...
        if self.robot:
            ...
            self.digits_left.add_object(urdf_fn=self.cfg[self.obj_name]["urdf_path"],
                                        obj_id=self.objects_scene[self.obj_name],
                                        globalScaling=self.cfg[self.obj_name]["global_scaling"])
            # right
            self.digits_right.add_camera(self.robot, self.franka_panda.digit_links[1])
            self.digits_right.add_object(urdf_fn=self.cfg[self.obj_name]["urdf_path"],
                                         obj_id=self.objects_scene[self.obj_name],
                                         globalScaling=self.cfg[self.obj_name]["global_scaling"])
        ...
        for i in range(1, num_obs + 1):
            self.objects_scene[f"obs{i}"] = self.bullet_client.loadURDF(
                fileName=self.cfg[f"scene{scene_id}"][f"obs{i}"]["urdf_path"],
