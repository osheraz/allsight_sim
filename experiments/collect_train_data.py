...
def main(cfg):
	...
	import tacto
	bg_left = np.load('./conf/bg_D20479.npy')
	bg_right = np.load('./conf/bg_D20510.npy')
	digits_left = tacto.Sensor(**cfg.tacto, background=bg_left)
	digits_right = tacto.Sensor(**cfg.tacto, background=bg_right)

	sim_id = px.init() # p.connect(p.GUI)
	...
	sim.digits_left = digits_left
	sim.digits_right = digits_right

	T = px.utils.SimulationThread(real_time_factor=0.01)
	T.start()
	...
	while k < NUM_TRAIN_SAMPLES:
		...
		if d_bad_grasp < 0.1:
			sim.reset_buffers()
			_, _, sim_frame, _, _  = p.getCameraImage(240, 240,viewMatrix,projectionMatrix)
