...
class Object_3D_scene:
    ...
    def show_object_contact(self, idx_points_contact):
        ...
        mesh.interpolateDataFrom(pts_external_contact, N=2, on="points", kernel='gaussian').cmap('viridis', vmin=0.0,
                                                                                                 vmax=1.0)
    ...
    def show_scene_mesh_contact(self, idx_points_contact, d_collision):
        ...
        idx = np.array([np.argmin(np.linalg.norm(non_convex_pc - pts_obj[idx_points_contact[j]], axis=1)) for j in
                        range(len(idx_points_contact))])
        idx_d = np.array([np.min(np.linalg.norm(non_convex_pc - pts_obj[idx_points_contact[j]], axis=1)) for j in
                          range(len(idx_points_contact))])

        pocc = np.zeros(self.num_pts_pc[0])
        idx_points_contact = idx[np.where(idx_d < 1e-3)]
        idx = list(
            [np.where(np.linalg.norm(non_convex_pc - non_convex_pc[idx_points_contact[j]], axis=1) < 2e-3)[0] for j in
             range(len(idx_points_contact))])
        ...
        mesh.interpolateDataFrom(pts_external_contact, N=3, on="points", kernel='gaussian').cmap('viridis', vmin=0.0,
                                                                                                 vmax=1.0)
